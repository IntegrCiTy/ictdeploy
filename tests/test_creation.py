...
def test_basic_files_are_in_nodes_folders(fix_volume):
    for node in fix_volume.edit.nodes.index:
        files = {
            "empty_file_for_testing_purpose.txt",
            "base_wrap.py",
            "init_values.json",
            "config_file.json",
        }
        assert (
            set(os.listdir(os.path.join(fix_volume.deploy.TMP_FOLDER, node))) == files
        )
...
def test_init_values_file_contains_all_data(fix_volume):
    for node in fix_volume.edit.nodes.index:
        with open(
            os.path.join(fix_volume.deploy.TMP_FOLDER, node, "init_values.json")
        ) as json_data:
...
def test_node_config_file_contains_all_data(fix_volume):
    for node in fix_volume.edit.nodes.index:
        with open(
            os.path.join(fix_volume.deploy.TMP_FOLDER, node, "config_file.json")
        ) as json_data:
        ...
        assert {
            ...
            "obnl.local.node.",
            "obnl.data.node.",
        }.issubset(config_file["queues"])
        assert {"obnl.simulation.node.", "obnl.local.node."}.issubset(
